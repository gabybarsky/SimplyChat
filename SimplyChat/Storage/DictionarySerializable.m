...
// utility to help serialize dictionary to a JSON string by converting data to strings
// that the NSJSONSerialization has trouble converting
@implementation NSDictionary (getSafeDictionary)
-(NSDictionary *)getSafeDictionary
{
    ...
    for(id key in self)
    {
        ...
        if ([object isKindOfClass:[NSDate class]])
        {
            ...
            [pDictionary setObject:result forKey:key];
        }
        else
        {
            [pDictionary setObject:object forKey:key];
        }
    }
    return pDictionary;
}
...
{
    NSError *error;
    NSDictionary *safeDictionary=[[self dictionaryValue] getSafeDictionary];
